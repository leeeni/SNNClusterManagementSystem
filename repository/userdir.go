...
func CheckDirExistByUsername(username string) (bool, error) {
	return Has(UserDirCollection, bson.M{"username": username})
}

// 检索
func CheckDirExistByDirName(dirname string) (bool, error) {
	return Has(UserDirCollection, bson.M{"dirname": dirname})
}

// 增
func InsertUserDir(userdir *UserDir) error {
	return Insert(UserDirCollection, userdir)
}

// 查
func GetUserDirByUsername(username string) UserDir {
	userdir := UserDir{}
	GetByQ(UserDirCollection, bson.M{"username": username}, &userdir)
	return userdir
}
...
func GetUserDirByDirname(dirname string) UserDir {
	userdir := UserDir{}
	GetByQ(UserDirCollection, bson.M{"dirname": dirname}, &userdir)
	return userdir
}

// 改
func UpdateUserDir(userdir UserDir) error {
	err := UserCollection.Update(bson.M{"_id": userdir.Uid}, userdir)
	return err
}

// 删
func DeleteUserDirByUserName(username string) error {
	err := UserDirCollection.Remove(bson.M{"username":username})
	return err
}
